...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const landing = getLandingPage('it', 'under-x', slug)
  if (!landing) return {}

  const canonicalUrl = `${BASE_URL}/it/offerte/${slug}`
  return {
    title: landing.title,
...
export default async function ItOffertePage({ params }: PageProps) {
  const { slug } = await params
  const landing = getLandingPage('it', 'under-x', slug)
  if (!landing) notFound()
  ...
  return (
    <LandingPageComponent
      ...
      canonicalUrl={`${BASE_URL}/it/offerte/${slug}`}
      homeUrl="/it"
