...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const landing = getLandingPage('it', 'best-price', slug)
  if (!landing) return {}

  const canonicalUrl = `${BASE_URL}/it/miglior-prezzo/${slug}`
  return {
    title: landing.title,
...
export default async function ItBestPricePage({ params }: PageProps) {
  const { slug } = await params
  const landing = getLandingPage('it', 'best-price', slug)
  if (!landing) notFound()
  ...
  return (
    <LandingPageComponent
      ...
      canonicalUrl={`${BASE_URL}/it/miglior-prezzo/${slug}`}
      homeUrl="/it"
