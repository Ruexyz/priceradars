...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const landing = getLandingPage(country, 'under-x', slug)
  if (!landing) return {}

  const canonicalUrl = `${BASE_URL}/en/${country}/deals/${slug}`
  return {
    title: landing.title,
...
export default async function EnDealsPage({ params }: PageProps) {
  const { country, slug } = await params
  ...
  return (
    <LandingPageComponent
      ...
      canonicalUrl={`${BASE_URL}/en/${country}/deals/${slug}`}
      homeUrl={`/en/${country}`}
