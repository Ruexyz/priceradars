...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const landing = getLandingPage(country, 'brand', slug)
  if (!landing) return {}

  const countryConfig = countries[country as CountryCode]
  const canonicalUrl = `${BASE_URL}/en/${country}/brand/${slug}`
  return {
    title: landing.title,
...
export default async function EnBrandPage({ params }: PageProps) {
  const { country, slug } = await params
  ...
  return (
    <LandingPageComponent
      ...
      canonicalUrl={`${BASE_URL}/en/${country}/brand/${slug}`}
      homeUrl={`/en/${country}`}
